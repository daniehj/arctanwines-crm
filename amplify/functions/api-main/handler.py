...
@app.post("/db/migrate")
def run_migrations():
    """Run database migrations using Alembic with local testing first"""
    try:
        import subprocess
        import os
        from pathlib import Path
        ...
        def run_alembic_command(command_args: list, local_db: bool = False):
            """Run Alembic command with environment configuration"""
            try:
                # Set up environment for local testing
                if local_db:
                    os.environ['DATABASE_URL'] = 'sqlite:///./test_migration.db'
                    print(f"🧪 Testing migration locally with SQLite")
        ...
        # Step 1: Test migration locally first
        print("🧪 Step 1: Testing migration on local SQLite database...")
        local_test = run_alembic_command(['upgrade', 'head'], local_db=True)
        migration_results.append({
            "step": "local_test",
            "description": "Test migration on local SQLite",
            "result": local_test
        })
        
        if not local_test['success']:
            return {
                "status": "error",
                "message": "Local migration test failed - aborting production migration",
                "error": local_test['stderr'],
                "details": migration_results
            }
        
        print("✅ Local test passed! Proceeding with production migration...")
        
        # Step 2: Run production migration
        print("🚀 Step 2: Running production migration...")
        prod_migration = run_alembic_command(['upgrade', 'head'], local_db=False)
        migration_results.append({
            "step": "production_migration", 
            "description": "Run migration on production database",
            "result": prod_migration
        })
        
        if prod_migration['success']:
            return {
                "status": "success",
                "message": "Migration completed successfully (tested locally first)",
                "details": migration_results
            }
        else:
            return {
...
@app.post("/db/alembic/test-local")
def test_local_migration():
    """Test migrations on local SQLite database only"""
    try:
        import subprocess
        import os
        from pathlib import Path
        ...
        try:
            ...
            return {
                ...
                "message": "Local migration test completed",
                "stdout": result.stdout,
                "stderr": result.stderr,
                "local_testing": True,
                "database_file": "test_migration.db"
            }
        finally:
            os.chdir(original_cwd)
...
@app.post("/db/migrate-legacy")
def run_legacy_migrations():
    """Legacy manual migration system (fallback)"""
    try:
        # Legacy manual migration system (kept as fallback)
        db = get_db_session()
        tables_created = []
